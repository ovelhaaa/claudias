...
export function App() {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      ...
      const frame = matrix.process(params, sources, dt, meter.input, meter.output, { freeze, gate, trigger: false });
      engine.setParams({ ...frame.params, freeze: frame.freeze, gate: frame.gate, playback_mode: playbackMode, quality, bypass });
    }, 20);
    return () => clearInterval(id);
  }, [engine, matrix, params, sources, meter.input, meter.output, freeze, gate, playbackMode, quality, bypass]);
